...
PID_FILE=$CUR_PATH/logger.pid

if [ -z "$3" ]; then
  CONFIG=$CUR_PATH/common/config.logger
else
  CONFIG=$CUR_PATH/$3
fi
...
function view(){
  cd $CUR_PATH
  debug_cfg=$CUR_PATH/common/debug_cfg.log
...
function stop(){
  ...
  if [ -z "$exist_pid" ] ;then
    ...
    echo -n $"$pid loggerserver will killed"
    killproc -p $PID_FILE
    echo
  fi
}
