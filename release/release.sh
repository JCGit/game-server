...
function upload(){
    release_dir="${WORK_DIR}upload"
    bak_dir="${WORK_DIR}bak"
    pro_dir="${WORK_DIR}${GAME_NAME}"
    ssh $USER@$HOST "mkdir -p $release_dir"
    scp $FILE $USER@$HOST:${release_dir}
    if [ "$OPTION" != "copy" ]; then
        ssh $USER@$HOST "cd $WORK_DIR
        mkdir -p $bak_dir
        tar cvf $bak_dir/$GAME_NAME-$(date '+%Y-%m-%d_%H_%M_%S').tar.gz ./$GAME_NAME --exclude=log/*
        tar xvf $release_dir/$(basename $FILE) -C ${pro_dir}
        bash $pro_dir/start/restart.sh"
    fi
}
